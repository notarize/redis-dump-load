...
def client(host='localhost', port=6379, password=None, db=0,
                 unix_socket_path=None, encoding='utf-8', secure=True):
...
def dump(fp, host='localhost', port=6379, password=None, db=0, pretty=False,
         unix_socket_path=None, encoding='utf-8', keys='*', secure=None):
    ...
    if pretty:
        ...
        fp.write(dumps(host=host, port=port, password=password, db=db,
            pretty=pretty, encoding=encoding, keys=keys, secure=secure))
        return
...
class StringReader(object):
    ...
    def handle_response(response, pretty, encoding):
        ...
        #try:
        decoded_response = response.decode(encoding, errors='ignore')
        #except:
        #    pass
        return decoded_response #response.decode(encoding)
...
def main():
    ...
    parser.add_option('-S', '--secure', help='Use SSL')
